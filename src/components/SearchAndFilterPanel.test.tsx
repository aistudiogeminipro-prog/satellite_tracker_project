import { render, screen, fireEvent } from '@testing-library/react';
import { describe, it, expect, vi } from 'vitest';
import { SearchAndFilterPanel } from './SearchAndFilterPanel';
import { TLEData } from '../types/satellite';

describe('SearchAndFilterPanel', () => {
  const sampleSatellites: TLEData[] = [
    {
      name: 'ISS (ZARYA)',
      noradId: '25544',
      line1: '1 25544U 98067A   24120.50000000  .00016717  00000-0  30000-3 0  9993',
      line2: '2 25544  51.6400 208.9100 0004000  90.0000 270.0000 15.49000000123457',
    },
    {
      name: 'SENTINEL-1A',
      noradId: '39634',
      line1: '1 39634U 14016A   24120.50000000  .00000123  00000-0  10000-4 0  9991',
      line2: '2 39634  98.1800 150.2300 0001200 100.0000 260.0000 14.59000000543210',
    },
  ];

  it('filters satellites by search input', () => {
    const onSelect = vi.fn();
    render(<SearchAndFilterPanel satellites={sampleSatellites} onSelectSatellite={onSelect} />);

    expect(screen.getByTestId('search-filter-panel')).toBeInTheDocument();
    expect(screen.getByText('搜尋結果 (2)')).toBeInTheDocument();

    const input = screen.getByTestId('search-input');
    fireEvent.change(input, { target: { value: 'SENTINEL' } });

    expect(screen.getByText('搜尋結果 (1)')).toBeInTheDocument();
    expect(screen.getByTestId('sat-item-39634')).toBeInTheDocument();

    fireEvent.click(screen.getByTestId('sat-item-39634'));
    expect(onSelect).toHaveBeenCalledWith(
      expect.objectContaining({ noradId: '39634' }),
      expect.objectContaining({ category: 'SAR' })
    );
  });
});
